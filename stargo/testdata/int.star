...
# int types
assert.eq('go.type<int>', type(go.int))
assert.eq('go.type<int8>', type(go.int8))
assert.eq('go.type<int16>', type(go.int16))
assert.eq('go.type<int32>', type(go.int32))
assert.eq('go.type<int64>', type(go.int64))
...
# TODO: methods.  parser.Mode.
# TODO: comparisons. Truth value. Hashable.


load("go", "stargo_test")
assert.eq('go.type<stargo_test.myint16>', type(stargo_test.myint16))
...
y.Incr()
assert.eq(3, y.Get())
# TODO: adding *expr into the grammar creates ambiguity with calls f(*args).
# assert.eq(3, (*y))
